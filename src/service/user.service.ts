...
export async function updateUserById(_id: string, data: Partial<User>) {
  idSchema.parse({ _id });
  ...
  if (Object.keys(safeData).length === 0) {
    throw new BadRequest("At least one field must be provided to update user");
  }
...
export async function isUserIdHasCorrectRole(userId: string, role: string) {
  return (await userRepository.getUserById(userId))?.role === role;
}
